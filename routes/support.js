...
router.get('/support/:id', async (req, res) => {
    try {
        ...
        if (foundComplaint.status === 'recieved') {
            foundComplaint.status = 'started';
...
router.put('/support/:id', function (req, res) {
    ...
    Complaint.findByIdAndUpdate(req.params.id, updatedComplaint, { new: true })
        .populate('author.id')
                ...
                if (foundComplaint.status == 'Open') {
                    async function main() {
                        ...
                        let info = await transporter.sendMail({
                            ...
                            text:
                                'Hi ' +
                                foundComplaint.author.id.username +
                                ',\n\n' +
